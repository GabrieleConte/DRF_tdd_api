...
class PublicUserApiTests(TestCase):
    """
    Test Public features of the API (no login required)
    """
    ...
    def password_too_short_error(self):
        ...
        self.assertEquals(res.status_code, status.HTTP_400_BAD_REQUEST)
        user_exists = \
            get_user_model().objects.get(email=payload['email']).exists()
    ...
    def test_create_token_badcredentials(self):
        """test error if credentials are invalid"""
